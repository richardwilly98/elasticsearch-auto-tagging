...
public class TransportAutoTaggingAction extends TransportSingleCustomOperationAction<AutoTaggingRequest, AutoTaggingResponse> {
    ...
    protected AutoTaggingResponse shardOperation(AutoTaggingRequest request, int shardId) throws ElasticSearchException {
        try {
            logger.info("shardOperation - {} - {} - {} - {}", request.getIndex(), request.getType(), request.getId(), request.getContent(), request.getField());
//            GetResponse getResponse = client.prepareGet(request.getIndex(), request.getType(), request.getId())
//                    .setFields(request.getContent()).get();
            SearchResponse searchResponse = client.prepareSearch(request.getIndex()).setTypes(request.getType()).addField(request.getContent()).setQuery(QueryBuilders.idsQuery(request.getType()).ids(request.getId())).get();
            if (searchResponse.getHits().getTotalHits() > 0) {
                
                logger.info("SearchResponse - {}", searchResponse.getHits());
                String text = searchResponse.getHits().getAt(0).getFields().get(request.getContent()).getValue();
//                Object text = getResponse.getField(request.getContent()).getValue();
                if (text == null) {
